...
export default async function ContentTypeEntriesPage({ params }: { params: { contentTypeKey: string } }) {
  ...
  const { data: type } = await supabase.from("content_types").select("id,key,label,schema_json").eq("key", params.contentTypeKey).single();
  if (!type) notFound();

  const { data: entries } = await supabase.from("content_entries").select("id,title,status,updated_at,content_json").eq("content_type_id", type.id).order("updated_at", { ascending: false });
