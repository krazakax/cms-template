    ...
    <div className="space-y-4">
      {fields.map((field) => {
        ...
        if (field.type === "object" && field.fields) {
          return <div key={key} className="rounded border p-3"><h4 className="mb-2 font-medium">{field.label}</h4><SchemaFieldRenderer fields={field.fields} register={register} parentKey={key} watch={watch} mediaOptions={mediaOptions} /></div>;
        }
          ...
          <label key={key} className="block space-y-1">
            ...
            {field.type === "textarea" ? (
              ...
              <div className="space-y-2">
                <select className="w-full rounded border px-2 py-2 text-sm" {...register(key)}>
                ...
                <Input placeholder="Or paste image URL" {...register(key)} />
